...
import { InventoryItem, ProcurementOrder, Supplier } from '@/lib/types';
...
export default function ManagerProcurementPage() {
  ...
  const fetchSuppliers = useCallback(async () => {
    try {
      ...
      const data = res.data.results || res.data;
      const active = (Array.isArray(data) ? data : []).filter((s: Supplier) => s.is_active);
      setSuppliers(active);
      // Auto-select first supplier (should be "5 Ways")
      if (active.length > 0 && !selectedSupplier) {
        setSelectedSupplier(active[0].id);
      }
    } catch {
  ...
  const loadItems = useCallback(async () => {
    try {
      const res = await api.get('/inventory/items/', { params: { category: 'procurement', is_active: 'true' } });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post('/procurement/orders/', {
        ...
        notes,
        items: filled.map((r) => ({ item: r.item_id, quantity: Number(r.quantity), price: r.price })),
      });
      toast.success(`Order submitted — ${filled.length} items`);
      setShowForm(false);
  ...
  return (
    <ProtectedRoute allowedRoles={['shop_manager']}>
        ...
        <div className="bg-white border rounded-xl p-6 mb-6">
          ...
          <div className="flex justify-end mt-4">
            <button
              ...
              disabled={submitting || filledCount === 0 || !selectedSupplier}
      ...
      {loading ? <div className="text-gray-400">Loading...</div> : orders.length === 0 && !showForm ? (
        <EmptyState message="No supplier orders yet. Click '+ New Order' to create one." />
      ) : !showForm && (
            ...
            <div key={order.id} className="bg-white border rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <span className="text-gray-500 ml-2 text-sm">— {order.supplier_name || supplierName(order.supplier)}</span>
                </div>
                <StatusBadge status={order.status} />
              </div>
              {order.notes && <p className="text-sm text-gray-500 mb-2">{order.notes}</p>}
              ...
              {order.total && (
                <p className="text-sm font-medium text-brand-600 mt-2">Total: A${Number(order.total).toFixed(2)}</p>
              )}
            </div>
          ))}
        </div>
